...
* `EventToast` — 480 px max dark panel with a white tab, centred at 20 % from the top, four seconds each, queued: roll results (with the event die), event cards, robber and pirate moves, island bonuses, raids, the boot, barbarian attacks, Defender awards, metropolises, opponents' progress cards. The connection notice uses the same chrome at 12 %.
