...
- `SupabaseDriver` prefers the server's legal list when the `game_views` row carries one (`legal jsonb`, migration `20260913000000_game_views_legal.sql`; `refreshViews` writes it), so the Spy prompt works online; the hotseat driver already uses the engine's full list.
