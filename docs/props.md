...
- **GLB pieces (settlement, city, road):** the same rule. `settlement.glb` (cottage) and `city.glb` (keep with a flag) are split by raw-model Y into base / middle / top zones (`PIECES` in `loadPiece.ts`): base and top in the player colour, the middle in plaster `#F4EFE6` (settlement) or stone `#7A8798` (city); scaled to the procedural heights 0.18 R and 0.34 R. `road.glb` (a bridge, 1.0 long on its Z, 0.46 wide, 0.10 thick) is wholly in the player colour, scaled uniformly so Z spans the hex edge (1 R), lifted by half its thickness and turned to lie along the edge. Each figure keeps its procedural version as the fallback.
...
- **Knight (Phase 11):** three levels, all on a base ring in player color, all in player color (inactive: desaturated grey `#9A9A94`, base ring dimmed). Shield carries 1/2/3 pip dots for the level.
