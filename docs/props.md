...
Props avoid the center recess and a 0.06 R margin at the slab edge. Layout positions are seeded; the counts below are the defaults at High quality (Medium ×0.7, Low ×0.4).

**Every prop is drawn at `PROP_SCALE` (0.72) of the sizes below**, so the tallest scenery (a cabin's ridge, a windmill's cap, a peak) stays under a settlement (0.36) and the pieces are the biggest things on the land. Each terrain's landmark (the cabin, the hut, the windmill, the kiln, the mine, the sluice) stands on a seeded share of its tiles, not every one — `hasLandmark(hex, terrain)` rolls once per tile on its own stream, so which tiles carry one never changes with the quality density: cabin 15 % (a house on a forest hex is what a settlement looks like), hut, windmill, kiln and mine 40 %, sluice 50 %, the cactus always. A board where every forest had a cabin and every field a windmill read as a village on every hex.
...
- **GLB pieces, world units (hex edge = 1, x width × y height × z length; `PIECES` in `loadPiece.ts`, every model lifted by its own min Y):** one whole-city model replaces the city at an upgraded vertex, and each stands taller than the plain city's 0.62 — `city_walled.glb` 0.72 × 0.68 × 0.72 when the vertex has a wall (zones −0.32 / 0.20: base disc and flag player-coloured, ring and tower grey), `metropolis.glb` 0.62 × 0.78 × 0.62 at a metropolis (zones −0.42 / 0.38: base and crown in the player colour, body grey `#8a8f99`), and `metropolis_walled.glb` 0.72 × 0.84 × 0.72 when it is both (zones −0.42 / 0.40, read off the model: its base disc is radius 0.45 against the wall's 0.39, and nothing at all sits between 0.34 and 0.42). The two walled models share the 0.72 footprint so the wall ring reads the same size on a city and a metropolis. `ship.glb` 0.26 × 0.32 × 0.76 on sea edges (length on its local X, turned to Z, then the road orientation; only the sail above raw Y 0.20 is player-coloured, the rest wood `#8b6a45`, hull bottom at raw Y −0.455 on the water); `barbarian_ship.glb` 0.30 × 0.28 × 0.65 on the table track (sail above 0.24 dark red `#7a2a2a`, hull near-black `#1a1a1a`); `pirate.glb` 0.42 × 0.38 × 0.38 at the sea hex (sail above 0.18 dark red, rest near-black, laid along the hex's flat sides); `merchant.glb` 0.36 × 0.36 × 0.36 at the hex like the robber, one warm neutral `#c9b58a`; `knight_1/2/3.glb` 0.28 × 0.40 × 0.28, 0.31 × 0.44 × 0.31, 0.35 × 0.48 × 0.35 (a knight stands with a settlement; at 0.26 it was the smallest thing on the land) (zones −0.42 / 0.10, −0.33 / 0.22, −0.38 / 0.19: base and top player-coloured, body grey; the level is which model loads, there are no rank pips; inactive knights have every zone colour × 0.55 and roughness 1); `port_sign.glb` 0.30 × 0.36 × 0.28 at the pier's seaward end facing the land hex, wood. The settlement, city and robber heights are the procedural ones × 2 (0.36, 0.62, 0.56). The original Meshy exports are archived under their generated names in `docs/art/models/`, with a table mapping each to the piece it serves; nothing loads them, the served copies are `apps/web/public/models/`.
...
- All pieces are 2× their "true" scale (`FIGURE_SCALE`, `PIECE_SCALE`) and the props 0.72× theirs (`PROP_SCALE`), so the pieces read first from the default camera: a settlement is the biggest house on its corner and a level-1 knight is never lost among the sheep.
- **Port disc** (`Harbor.tsx`): the identifier the table reads is a number-token-sized disc (radius 0.24) on the sea 0.62 R beyond the coast edge's midpoint, its face the resource's colour as a broad ring (ink for 3:1) round a bone face with the ratio large and the resource's short name under it (`portTokenTexture`), turned to face the camera like the number tokens, a gold ring on it once the viewer settles the port, and three plank dashes from the disc toward each of the port's two vertices so the corners it serves are drawn on the water. The pier and its hanging sign stay as dressing.

## 5. Token, harbor, dice

- **Token:** cylinder radius 0.26 R height 0.05 R (up from the 0.20 / 0.04 first drawn: at the default camera the smaller token was ~22 px across on a 1080p frame and its numeral unreadable), clay `#C9976A` sides and rim (wax red `#C0392B` for 6/8), with a raised rim 0.01 R high and a recessed face in pale bone `#F4EBD6`; the numeral as a canvas-baked sprite on the face in ink (wax red on 6 and 8), pips as small dots below it. Bone under ink is about 12:1 against the 3:1 of the earlier clay-under-ink face.
...
- **Merchant (Phase 11):** rotund figure — sphere body in green `#4E8A4A`, sphere head, round flat hat, small ledger box in one hand — beside a two-wheel cart carrying two crates; on a green base ring. Not player-colored. The GLB is zoned the same way: green base disc and hat over a warm tan coat.
- **Pirate ship (Phase 9):** dark hull `#3A3532`, single black square sail, small black pennant at the masthead, no base ring.
- **Barbarian longship (Phase 11 fleet):** long low dark hull 0.9 R, dragon-head prow (stacked cones), a row of 6 round shields along each side, one square tattered grey sail (`#8A857D`, notched edges), no base ring. Sails along the fleet's lane: its own strip of dark water on a walnut-edged slab along the board's bottom-left edge, seven markers from the open sea to a stone landing (a squat tower on a red disc) at the coast end.
- **Improvement books (Phase 11):** each seat's three city-improvement tracks as flat books beside its piece pile, covers in the track colours (`TRACK_COLOR`), the level as raised bone studs down the cover (a dark recess for each level still to build), a gold seal on a track whose metropolis the seat holds. The viewer's own books open the improvement sheet.
- **Piece piles:** each seat's unplaced roads, settlements, cities and ships or level-1 knights are the same GLB figurines as on the board at 0.7 of their size, clustered by the seat's table edge (`src/board/props/PiecePiles.tsx`): roads side by side like stacked planks, then a row per kind.
...
- Post (High and Medium): vignette offset 0.55 darkness 0.18. No tilt-shift: the blur it laid across the top and bottom of the frame softened the far side of the board and the near piles, and on a full-bleed canvas it read as the scene being out of focus rather than as a miniature.
...
- **Post-processing** is the vignette alone (0.18 at offset 0.55), which shades the corners without reaching the board. The tilt-shift blur was removed (§6).
- **GLB city colouring.** The city models are one mesh each, so the colour zones are cut from the geometry (`loadPiece.ts`): base disc and flag in the player colour by height as before, and the middle band split again by which way each triangle faces — level faces (|ny| > 0.85: wall tops, battlements, terraces) in a light stone `#DAD3C3`, sloped faces (0.3 < |ny| ≤ 0.85: the roofs) in the player colour darkened to 0.72, the rest (the walls) in a warm sandstone `#B3A88E`. Before this the plain, walled and metropolis cities were a single blue-grey mass under a coloured base.
