...
- Placement rules (`legal.ts`) only offer land vertices and land edges (`isLandVertex`, `isLandEdge`); `setupAllowed` applies a scenario's setup restriction (Phase 9).
...
- `model.ts` is a pure reducer over `{ def, tool, terrain, symmetry, selected, gridRadius, dirty, boardId }` with undo/redo (`historyReduce`, 200 steps; view-only actions are not recorded). Tools: **frame** (a brush of one kind, `frameKind`: Land (L), Sea (S) or Frame; click or drag paints it, right-click erases, shift-drag paints a rectangle, mirror/rotate symmetry; the older `cycleCell` action remains for scripts), **terrain** (paint one terrain by click or drag, keys 1–7; an empty, sea or frame cell becomes land; painting a wasteland drops its token), **token** (select a hex and type 2–12; the tray shows what the balanced pool still owes), **harbour** (click a coastal edge to cycle none → 3:1 → 2:1 per resource → none). "Auto tokens" runs the engine's balanced generator and pins the result as fixed tokens; "Auto harbours" spaces `harborCount` harbours along the coast. Templates load any built-in frame.
...
- `LAND_SPLIT`: `save-board` still accepts islands (`allowIslands: true`), but the editor validates a plain board exactly as the picker, `create-lobby` and `createGame` do, so a split board shows the error ("join them, or enable Tides") instead of saving fine and then appearing greyed out in the picker. Switching Tides on in the Scenario section is the way to keep islands.
- Painting: the camera is disabled for the length of a paint drag (`CameraRig enabled`), so left-drag never orbits or pans the diorama while it paints; the hovered cell shows a ring.
- Saving: `Save` waits for the session lookup (it used to make a device draft when pressed before the session arrived), the toast says where the board went, and `/boards` offers "Save to my library" for device drafts of a signed-in player. The home page links to `/boards`.
