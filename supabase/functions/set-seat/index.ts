// Edge Function `set-seat` (docs/phase4.md §3 / docs/phase5.md §1). Thin: auth and
...
Deno.serve(handle(async ({ sql, userId, body }) => {
  await setSeat(sql, { gameId: str(body, "gameId"), userId, color: body.color as never, name: typeof body.name === "string" ? body.name : undefined });
  return {};
}));
