 ...
 * black-sailed ship that eases (600 ms) one dot per fleet step, and two pills
...
function FleetShip({ from, to, key_ }: { from: { x: number; z: number }; to: { x: number; z: number }; key_: number }) {
  ...
  return (
    <group ref={group} position={[to.x, 0.06, to.z]} scale={1.1} name="fleet-ship">
      <mesh position={[0, 0.04, 0]} castShadow>
        <boxGeometry args={[0.16, 0.08, 0.44]} />
        <meshStandardMaterial color="#2a2622" flatShading />
      </mesh>
      <mesh position={[0, 0.24, 0]}>
        <cylinderGeometry args={[0.01, 0.01, 0.34, 5]} />
        <meshStandardMaterial color="#211d19" />
      </mesh>
      <mesh position={[0, 0.22, 0.07]} rotation={[0, 0, 0]}>
        <planeGeometry args={[0.02, 0.3]} />
        <meshStandardMaterial color="#1f1a17" side={THREE.DoubleSide} />
      </mesh>
      <mesh position={[0.08, 0.2, 0.06]} rotation={[0, Math.PI / 2, 0]}>
