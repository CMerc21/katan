...
* `src/board/props/` — on-table props rendered inside the diorama: `layout.ts` (pure positions from the board bounds), `BankDecks`, `PiecePiles` (the GLB figurines at 0.7 scale), `BarbarianTrack` (the fleet's lane along the bottom-left edge), `ImprovementBooks` (Crown: the tracks as books beside each pile).
