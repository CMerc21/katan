...
* react-three-fiber board in `apps/web/src/board3d` (tiles, seeded props, figurines, camera rig, raycast interaction layer with accessible overlay buttons, in-scene dice and robber animations, quality presets); the SVG board moved to `board2d` for thumbnails. Details in `docs/phase7-5.md`.
...
* `src/board3d/` — the diorama used in play, built to the tile and prop brief in `docs/props.md`: `Board3D` (Canvas, lights), `layout3d` (world coords = engine geometry, y → z), `slab.ts` (slab geometry: layers, recess, relief, jitter), `palette.ts` (the diorama's colours), `Tiles` (slabs, tokens, sea ripple), `Props` + `props.ts` (seeded per-terrain placement rules, pure) + `propModels.ts` (the prop GLB table: fit, colour, sink; one InstancedMesh per kind, procedural stand-in and a one-time log for any kind without a model), `Pieces`, `islands.ts` (mesh islands: welded connected components in a deterministic order, and attribute-preserving triangle regrouping; pure, run by `scripts/inspect-model.ts`), `loadPiece` (GLB figurines from `public/models/<name>.glb` via GLTFLoader for every piece: robber, settlement, city, road, metropolis, metropolis_walled, city_walled, ship, barbarian_ship, pirate, merchant, knight_1..3, port_sign; geometry cached by name, normals computed, `PIECES` config in world units with hand-mapped `playerColorIslands` (height-band `zones` as the fallback below three islands), a length-axis turn and a uniform or per-axis fit, the baked texture on non-player parts when an export has one, flat-shaded materials coloured by the caller, foot lifted by the model's own min Y; `usePiece` returns `null` so callers keep the procedural figure as a fallback), `Crown3d` (knights, `CrownCityFigure` for walled cities and metropolises — one model per combination, all larger than the plain city, the walled pair sharing a footprint — merchant, longship), `Wayfarers3d`, `Harbor`, `Camera`, `Interaction` (layer-1 raycast targets), `Effects3d` (dice, robber, projector), `quality` (the three presets: High is Medium plus a 2048 shadow map and the rim light; no auto tier, no watchdog), `shadow.ts` (pure `fitShadowCamera`: the board's bounding box projected into the key light's basis, logged once), `outline.ts` (inverted-hull outlines on pieces and props, a fixed 0.008 world-unit offset along the normal via `onBeforeCompile`, never on tiles), `textures` (canvas sprites).
...
pnpm inspect:model <file.glb>   # print a model's mesh islands (id, triangles, Y range, radius, centroid) to map playerColorIslands by hand
