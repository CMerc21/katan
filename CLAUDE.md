...
* `src/board3d/` — the diorama used in play, built to the tile and prop brief in `docs/props.md`: `Board3D` (Canvas, lights), `layout3d` (world coords = engine geometry, y → z), `slab.ts` (slab geometry: layers, recess, relief, jitter), `palette.ts` (the diorama's colours), `Tiles` (slabs, tokens, sea ripple), `Props` + `props.ts` (prop geometry and seeded layouts, landmarks on a seeded share of tiles, drawn at `PROP_SCALE`), `Pieces`, `loadPiece` (GLB figurines from `public/models/<name>.glb` via GLTFLoader for every piece: robber, settlement, city, road, metropolis, metropolis_walled, city_walled, ship, barbarian_ship, pirate, merchant, knight_1..3, port_sign; geometry cached by name, normals computed, `PIECES` config in world units with Y-threshold colour zones, a length-axis turn and a uniform or per-axis fit, flat-shaded materials coloured by the caller, foot lifted by the model's own min Y; `usePiece` returns `null` so callers keep the procedural figure as a fallback), `Crown3d` (knights, `CrownCityFigure` for walled cities and metropolises — one model per combination, all larger than the plain city, the walled pair sharing a footprint — merchant, longship), `Wayfarers3d`, `Harbor` (pier plus the camera-facing port disc and dashes), `Camera` (one fixed table view for every seat, `LABEL_TURN` for camera-facing labels), `Interaction` (layer-1 raycast targets), `Effects3d` (dice, robber, projector), `environment` (the procedural PMREM image-based light on `scene.environment`), `quality` (presets, detection, watchdog), `textures` (canvas sprites).
...
* `src/hud/` — the Phase 12 HUD (board-first: every affordable build is a target at rest, the cost card is a closed-by-default reference behind a Build toggle): `HudLayer` (grid, keys, toasts, panels), `PlayerBanner`, `ResourceTray`, `LeftRail` + `SidePanel` + `panels` (chat, emote, log, stats, rules, leave) + `SettingsMenu` (the settings body), `BuildCostCard`, `ActionButtons`, `CardsPanel`, `ModuleStrip` + `WayfarersActions`, `DiceWidget`, `TurnBanner`, `EventToast`, `HelpTip`, `primitives` (`Numeral`, `hud:*` events), `hudCopy.ts` (all HUD text), `model.ts` (pure: banner stats, cost rows, status line, dice history; unit tested), `icons/`. Tokens in `src/styles/hud.css`.
* `src/board/props/` — on-table props rendered inside the diorama: `layout.ts` (pure positions from the board bounds; seats counted from the viewer), `BankDecks`, `PiecePiles` (the GLB figurines at 0.7 scale, the viewer's on the near edge), `BarbarianTrack` (the fleet's chain of sea hexes off the board's top-left corner), `ImprovementCard` (Crown: each player's improvement chart as a card in front of their pile, the viewer's clickable).
